# Stake Belt Deployment Guide

This guide walks through deploying Stake Belt from a fresh clone to a working live demo. Two pieces deploy independently:
...
1. Builds the poll contract to `wasm32v1-none/release`.
2. Deploys the poll contract.
3. Writes `NEXT_PUBLIC_MAIN_CONTRACT_ID` into `.env.local`.
